...
export default class Playlist extends React.Component {
	...
	componentWillMount() {
		let bp = !this.props.opts ? 768 : this.props.opts.breakpoint ? (Math.abs(this.props.opts.breakpoint.maxWidth + (this.props.opts.offset && this.props.opts.offset.left ? this.props.opts.offset.left : 0) + (this.props.opts.offset && this.props.opts.offset.right ? this.props.opts.offset.right : 0))) : 768 ;

		this.breakpoint = window.matchMedia( `(max-width: ${bp}px)` );
	...
	setActiveTrack = track => {
		if (track.src === this.state.activeTrack) { return };
		if (window.audio && window.audio.active) { window.audio.active.pause() };
	...
	playlistBody() {
		...
		const styleOffsetOverides = Object.assign({}, { left: 0, right: 0 }, this.props.opts && this.props.opts.breakpoint && this.props.opts.breakpoint.removeOffset && this.state.isMobile ? {} : this.props.opts.offset );
